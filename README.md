...
- `OPENAI_TRANSCRIBE_MODEL` (default `whisper-1`; `gpt-4o-transcribe` is the alternative)
- `OPENAI_REALTIME_VOICE` (default `ballad`; any of marin, cedar, ballad, sage, verse, coral, alloy, ash, echo, shimmer. The Test Call panel can pick a voice per call. Accent comes from the instructions, which pin British English)
